...
VERSION=$(/usr/bin/curl -s "$SOURCE" | grep -e "update version" | awk -F\" '{print $2}' | sed 's/,/./')
...
/usr/bin/curl -s "$DOWNLOADURL" -o "$DOWNLOADPATH"

echo "Installing Adobe Flash Player..."
# Create a temp dir to mount disk image at
TMPMOUNT=$(/usr/bin/mktemp -d /private/tmp/"$UUID"_adobe_flash_"$VERSION"_tmpdir.XXXX)

# Mount disk image silently
/usr/bin/hdiutil attach "$DOWNLOADPATH" -mountpoint "$TMPMOUNT" -nobrowse -noverify -noautoopen 1> /dev/null
...
/usr/sbin/installer -dumplog -pkg "$INSTALLPKG" -target / 2> /dev/null

# Unmount disk image, delete tempdir and dmg
echo "Cleaning up up temporary files"
/usr/bin/hdiutil detach "$TMPMOUNT" 1> /dev/null
